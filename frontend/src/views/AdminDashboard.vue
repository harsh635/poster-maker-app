...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap">
  ...
  <div class="d-flex align-items-center gap-2">
    ...
    <button class="btn btn-outline-primary d-flex align-items-center gap-2" @click="openModal('create')">
    ...
    >
      ...
      <div v-if="showModal" class="modal-overlay">
        <div class="modal-content">
          <button class="btn-close position-absolute top-0 end-0 m-3" @click="closeModal" />
          <div class="row">
            <!-- Form -->
            <div class="col-md-6">
              <h5 class="fw-semibold mb-3">{{ modalMode === 'create' ? 'Add' : 'Edit' }} Template</h5>
              <input v-model="form.label" class="form-control mb-2" placeholder="Label" />
              <input v-model="form.occasion" class="form-control mb-2" placeholder="Occasion" />
              <input v-model="form.default_title" class="form-control mb-2" placeholder="Your Poster Title" />
              <input v-model="form.default_subtitle" class="form-control mb-2" placeholder="Your Subtitle" />
              <label class="form-label mt-2">Upload Background Image</label>
              <input type="file" class="form-control mb-3" @change="onImageUpload" />
...
<div v-if="showDeleteConfirm" class="delete-modal-backdrop">
  <div class="delete-modal-box">
    <h5 class="fw-semibold mb-3"><i class="fas fa-exclamation-triangle me-2" />Confirm Delete</h5>
    ...
    <div class="d-flex justify-content-end gap-2 mt-4">
      ...
      <button class="btn btn-danger btn-sm" @click="deleteTemplate(confirmDeleteId)">
        Yes, Delete
      </button>
    </div>
  </div>
...
const previewData = computed(() => ({
  title: form.value.default_title,
  subtitle: form.value.default_subtitle,
  name: '',
  phone: '',
  employeeName: '',
...
const modalTemplateDetails = computed(() => {
  const fallback = templates.value.find(t => t.id === currentTemplateId.value)
  return {
    ...form.value,
    image: imageFile.value ? URL.createObjectURL(imageFile.value) : fallback?.image || ''
  }
})
...
function openModal(mode, template = null) {
  ...
  if (mode === 'edit' && template) {
    ...
    form.value = { label: '', occasion: '', default_title: '', default_subtitle: '' }
